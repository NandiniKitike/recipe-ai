    ...
    try {
        ...
        if (decision.isDenied()) {
            if (decision.reason.isRateLimit()) {
                throw new Error(
                    `Monthly scan limit reached. ${isPro ? "Plesae contcat support if you need more scans."
                        : "Upgrade to Pro for unlimited scans!"

                    }`
                )
            }
            throw new Error("Request deniend by security system")
        }
...
- Common pantry staples are acceptable (salt, pepper, oil)`
        ...
        console.error("Error scaning pantryerror: ", text)
...
export async function saveToPantry(formData) {
    try {
        ...
        const ingredients = JSON.perse(ingredientsJson)
...
export async function addPantryItemsManually(formData) {
    try {
        ...
        throw new Error(error.message || "Failed to add item to pantry");
    }
...
export async function getPantryItems() {
    try {
        ...
        const data = await response.json();
        const isPro = user.subscripyionTier === "pro";
        return {
            success: true,
            items: data.data || [],
            scanLimit: isPro ? "unlimited" : 10,
